  ...
  return (
    <NodeShell status={d.status} color="blue" className="min-w-[300px] max-w-[400px]" agentRole={d.agentRole}>
      ...
      {editResult ? (
        ...
        <div className="text-center py-6 text-gray-500 text-xs space-y-2">
          <div>
            {d.status === 'pending'
              ? '等待视频生成完成...'
              : d.status === 'running'
          ...
          {d.status === 'completed' && (
            <button
              onClick={handleRegenerate}
              disabled={isRegenerating}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-500/15 text-orange-300 hover:bg-orange-500/25 text-[11px]"
            >
