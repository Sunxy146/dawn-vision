...
export function StoryboardRegenModal({
  ...
  // 关闭弹窗不中断请求：回调用 ref，卸载后仍能通知父组件更新图
  ...
  /** 只关 UI，后台 SSE 继续；完成后仍走 onComplete */
  const handleClose = () => {
    onCancel();
  };
  ...
  const handleRegen = async () => {
    ...
    // 关闭弹窗后仍回调「这一镜」；勿用最新 ref（可能已切到别的镜）
    ...
    try {
      ...
      if (!res.ok && !res.body) {
        ...
        if (mountedRef.current) setError(`请求失败 (${res.status}): ${txt.slice(0, 120)}`);
        return;
      }
      const reader = res.body?.getReader();
      if (!reader) {
        if (mountedRef.current) setError('无法读取响应流');
        return;
      }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (evt.type === 'status') {
              if (mountedRef.current) setStatus(evt.data?.message || '处理中...');
            } else if (evt.type === 'complete') {
              const newUrl = evt.data?.imageUrl;
              if (newUrl) {
                notifyComplete(newUrl, evt.data?.prompt || trimmed);
                return;
              } else if (mountedRef.current) {
                setError('上游未返新图');
              }
            } else if (evt.type === 'error') {
              if (mountedRef.current) setError(evt.data?.message || '重生失败');
            }
          } catch { /* skip malformed */ }
        }
      }
    } catch (e) {
      if (mountedRef.current) setError(e instanceof Error ? e.message : '重生失败');
    } finally {
  ...
  return (
    <div
      ref={dialogRef}
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150 outline-none"
      role="dialog"
      aria-modal="true"
      ...
      <div className="w-full max-w-2xl max-h-[90vh] rounded-2xl bg-[var(--cinema-surface)] border border-[var(--cinema-border-hi)] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-3 border-b border-[var(--cinema-border)] bg-[var(--cinema-surface-2)] flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {busy && (
              <span className="cinema-mono text-[10px] text-[var(--cinema-amber)] opacity-80 shrink-0">
                后台生成中 · 可先关闭
              </span>
            )}
          </div>
