...
export function ShotWorkshopTab({
  ...
  // 无视频时仍按分镜列出（否则「改 prompt 重生」按钮整页消失）
  const sortedShots = (
    videos.length > 0
      ? [...videos]
      : storyboards
          .filter((s) => typeof s.shotNumber === 'number')
          .map((s) => ({
            shotNumber: s.shotNumber as number,
            imageUrl: s.imageUrl,
            meta: { quality: 'storyboard' },
          }))
    ...
    <div className="space-y-5">
      ...
      {sortedShots.length === 0 ? (
        <div className="cinema-card">
          <EmptyState icon={Film} title="还没有分镜镜头" hint="完成主管线创作后回来，可在此重生分镜图 / 4K 重渲" />
        </div>
      ) : (
      ...
      {regenModalShot !== null && (
        <StoryboardRegenModal
          ...
          onComplete={(newUrl) => {
            const sn = regenModalShot;
            setSbOverrides((prev) => ({ ...prev, [sn]: newUrl }));
