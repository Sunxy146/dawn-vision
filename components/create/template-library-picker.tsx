...
export function TemplateLibraryPicker({
  ...
  const handleExportTemplate = (t: AnyTemplate) => {
    ...
    a.download = `dawnvision-template-${t.name.slice(0, 20)}-${Date.now()}.json`;
    a.click();
  ...
  const handleImportTemplate = async (file: File) => {
    try {
      ...
      if (parsed?.__dawnvisionTemplate !== 'v1') {
        alert('不是 DawnVision 模板 JSON. 请用 "导出" 按钮生成的文件.');
        return;
      }
