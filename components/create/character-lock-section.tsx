...
export function CharacterLockSection({ value, onChange }: Props) {
  // 始终内部维持 3 个槽位;onChange 时保留「已上传头像」的槽(名字可后填)
  ...
  // 外部 value 变化时合并进本地槽位(避免仅按 length 同步把「刚上传、尚未进父状态」的图冲掉)
  useEffect(() => {
    setSlots((prev) => {
      const next = prev.map((s) => ({ ...s }));
      // 用 value 覆盖已提交槽；多余 value 按序填空槽
      const used = new Set<number>();
      for (const v of value) {
        const byName = next.findIndex(
          (s, i) => !used.has(i) && s.name.trim() && s.name.trim() === v.name.trim(),
        );
        const idx =
          byName >= 0
            ? byName
            : next.findIndex((s, i) => !used.has(i) && s.imageUrl && s.imageUrl === v.imageUrl);
        const fallback = next.findIndex((s, i) => !used.has(i) && !s.imageUrl && !s.name.trim());
        const target = idx >= 0 ? idx : fallback;
        if (target < 0) continue;
        used.add(target);
        next[target] = { ...DEFAULT_SLOT, ...v };
      }
      return next;
    });
  }, [value]);

  const emitChange = (next: LockedCharacter[]) => {
    // 有头像即视为已锁；名字空时用占位，避免父组件丢掉导致「上传没反应 / LOCKED 一直 0」
    const ready = next
      .map((s, i) => {
        if (!s.imageUrl) return null;
        const name = s.name.trim() || `角色${String.fromCharCode(65 + i)}`;
        return { ...s, name };
      })
      .filter(Boolean) as LockedCharacter[];
    onChange(ready);
  };
  ...
  const updateSlot = (idx: number, patch: Partial<LockedCharacter>) => {
    setSlots((prev) => {
      ...
      if (next[idx].imageUrl && !next[idx].name.trim()) {
        next[idx].name = `角色${String.fromCharCode(65 + idx)}`;
      }
      emitChange(next);
      return next;
    });
    ...
    <div className="space-y-3">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-[11px] text-gray-400 [.cinema-page_&]:cinema-mono [.cinema-page_&]:tracking-wider">
          <span className="[.cinema-page_&]:hidden">🔒 先填角色名再上传更稳；只上传也会自动命名并锁定</span>
...
function CharacterCard({ slotLabel, slot, onUpdate, onClear }: CardProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      try {
        ...
        if (uploadFile.size < file.size && file.size > 10 * 1024 * 1024) {
          showToast({
            title: `已自动压缩 ${(file.size / 1024 / 1024).toFixed(1)}MB → ${(uploadFile.size / 1024 / 1024).toFixed(1)}MB`,
            type: 'info',
          });
        }
      } catch (e) {
        showToast({ title: e instanceof Error ? e.message : '图片处理失败', type: 'error' });
        return;
      }
      if (uploadFile.size > 10 * 1024 * 1024) {
        showToast({ title: '压缩后仍超过 10MB，请换一张更小的正脸照', type: 'error' });
        return;
      }
      ...
      if (!res.ok) {
        showToast({ title: body.error || body.message || `上传失败(${res.status})`, type: 'error' });
        return;
      }
      if (!body.url) {
        showToast({ title: '上传成功但未返回图片地址', type: 'error' });
        return;
      }
      onUpdate({ imageUrl: body.url });
      showToast({ title: '人脸已锁定到本槽位', type: 'success' });
  ...
  const handleUrl = async () => {
    ...
    try {
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast({ title: body.error || body.message || 'URL 抓取失败', type: 'error' });
        return;
      }
      if (!body.url) {
        showToast({ title: '抓取成功但未返回图片地址', type: 'error' });
        return;
      }
      onUpdate({ imageUrl: body.url });
