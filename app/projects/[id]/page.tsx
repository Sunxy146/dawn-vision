...
import { StoryboardRegenModal } from '@/components/project/storyboard-regen-modal';
...
function isVideoUrl(url: string, siblingUrls: string[] = []): boolean {
  if (!url) return false;
  if (url.startsWith('data:image')) return false;
  if (/\.(jpg|jpeg|png|gif|svg|webp|bmp|ico|tiff)(\?|#|$)/i.test(url)) return false;
  ...
  // 本地持久化 /api/serve-file?key=hash 无扩展名
  if (url.startsWith('/api/serve-file')) {
    const sibs = siblingUrls.length ? siblingUrls : [];
    // 同资产还有明确 mp4 外链 → 此 key 是视频副本
    if (sibs.some((s) => s && s !== url && /\.(mp4|webm|mov)(\?|#|$)/i.test(s))) return true;
    // 同资产只有图片外链 → 失败降级成了分镜图
    if (sibs.some((s) => s && s !== url && /\.(jpg|jpeg|png|gif|webp)(\?|#|$)/i.test(s))) return false;
    // 无旁证：视频 Tab 下的 serve-file 按可播放视频处理
    return true;
  }
...
/** 镜头视频资产是否「没有真视频」(仅分镜图 / animatic / 空) */
function isFailedVideoAsset(v: any, healthAnimatic?: Set<number>): boolean {
  if (v?.data?.isAnimatic === true) return true;
  if (healthAnimatic?.has(v?.shotNumber)) return true;
  const urls: string[] = Array.isArray(v?.mediaUrls) ? v.mediaUrls.filter(Boolean) : [];
  if (v?.persistentUrl && !urls.includes(v.persistentUrl)) urls.unshift(v.persistentUrl);
  if (urls.length === 0) return true;
  if (urls.some((u) => /animatic-\d+\.mp4/.test(String(u)))) return true;
  const playable = urls.find((u) => isVideoUrl(u, urls));
  return !playable;
}
...
export default function ProjectDetailPage() {
  ...
  const [regenSbShot, setRegenSbShot] = useState<{ shotNumber: number; imageUrl?: string; prompt?: string } | null>(null);
      ...
      while (reader) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (ev.type === 'batchDone') setRerenderMsg(`补渲完成:成功 ${ev.data?.ok}/${ev.data?.total}${ev.data?.fail ? `,失败 ${ev.data.fail}` : ''}${ev.data?.animatic ? `,降级 ${ev.data.animatic}` : ''}`);
          } catch { /* 跳过坏行 */ }
        }
      }
      // 重拉项目,刷新视频资产与成片
  ...
  const storyboards = dedupeAssetsByShotNumber(
    assets.filter((a: any) => a.type === 'storyboard'),
  );
  // v9.4.6: 一键成片闭环要用的「镜号→分镜 prompt」(防御式取, 取不到的镜面板会跳过)
  const shotPrompts = storyboards.map((s: any) => ({
    shotNumber: s.shotNumber || 0,
    prompt: s.prompt || (s.data && typeof s.data === 'object' ? (s.data.prompt || s.data.description) : '') || '',
  }));
  const videos = dedupeAssetsByShotNumber(
    assets.filter((a: any) => a.type === 'video'),
  );
  ...
  const tabs = [
    ...
    { key: 'workshop', label: '镜头工坊', icon: Scissors, count: Math.max(videos.length, storyboards.length) },
    ...
    <div className="cinema-page min-h-screen text-white">
      ...
      <nav className="sticky top-0 z-50 bg-[var(--cinema-surface)]/85 backdrop-blur-xl border-b border-[var(--cinema-border)]">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Link
              href={`/dashboard/create?resume=${encodeURIComponent(id)}`}
              className="cinema-btn cinema-btn-primary !px-3 !py-1.5 !text-[11px] inline-flex items-center gap-1.5"
              title="打开创作工坊（有分镜时可点「继续生成视频」）"
            >
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      {sb.mediaUrls?.[0] ? (
                        ...
                        <div className={`w-full ${frameClass} flex flex-col items-center justify-center gap-2 bg-[var(--cinema-surface-2)] cinema-mono text-[10px] opacity-70`}>
                          <span className="opacity-40">NO RENDER</span>
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              setRegenSbShot({
                                ...
                                prompt: sb.data?.description || sb.data?.planData?.description || sb.data?.prompt || scriptShot?.sceneDescription || '',
                              });
                            }}
                            className="cinema-btn cinema-btn-primary !px-2.5 !py-1 !text-[10px] inline-flex items-center gap-1"
                          >
                      ...
                      <div className="px-2.5 py-1.5">
                        <div className="flex items-center justify-between mb-0.5">
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              type="button"
                              onClick={() => setRegenSbShot({
                                shotNumber: sb.shotNumber,
                                imageUrl: sb.mediaUrls?.[0],
                                prompt: sb.data?.description || sb.data?.prompt || sb.data?.visualPrompt || scriptShot?.sceneDescription || '',
                              })}
                              title="重新生成这一镜分镜图"
                              className="p-1 rounded hover:bg-white/10 text-[var(--cinema-text-3)] hover:text-[var(--cinema-amber)]"
                            >
                              <RefreshCw className="w-3 h-3" />
                            </button>
                        ...
                        <p className="cinema-subhead text-[11px] line-clamp-2 opacity-85 leading-snug">
                          {(sb.data?.description || sb.data?.prompt || sb.data?.visualPrompt || '').slice(0, 60) || '——'}
                        </p>
          ...
          {activeTab === 'videos' && (
            ...
            {/* 失败/降级镜头批量补渲 — 含「只存了分镜图」的假视频资产 */}
            {(() => {
              const healthAnimatic = new Set(healthReport?.animaticShots || []);
              const degraded = videos.filter((v: any) => isFailedVideoAsset(v, healthAnimatic));
              if (degraded.length === 0 && !rerenderMsg) return null;
              const shotList = degraded.map((v: any) => v.shotNumber).filter(Boolean).join('、');
              return (
                <div
                  className="mb-4 px-4 py-3 rounded-lg border border-[var(--cinema-amber)]/40 bg-[var(--cinema-amber)]/15 flex flex-col sm:flex-row sm:items-center gap-3"
                  data-testid="batch-rerender-bar"
                >
                  <div className="flex-1 min-w-0 text-[13px] text-white/80">
                    <div className="font-semibold text-[var(--cinema-amber)]">
                      {degraded.length > 0
                        ? `${degraded.length} 个镜头没有真视频（镜 ${shotList}）`
                        : '补渲进度'}
                    </div>
                    <div className="text-[11px] text-white/45 mt-0.5">
                      用分镜图作首帧重新图生视频；额度不足时会失败，可稍后重试
                    </div>
                    {rerenderMsg && (
                      <div className="cinema-mono text-[10px] text-white/60 mt-1">{rerenderMsg}</div>
                    )}
                  </div>
                  {degraded.length > 0 && (
                    <button
                      ...
                      className="shrink-0 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[var(--cinema-amber)] text-black text-[13px] font-semibold hover:brightness-110 disabled:opacity-50"
                    >
                      {rerenderBusy ? '补渲中…' : `批量补渲 ${degraded.length} 镜`}
                    </button>
                  )}
                </div>
              );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {videos.map((v: any) => {
                const urls: string[] = Array.isArray(v.mediaUrls) ? v.mediaUrls.filter(Boolean) : [];
                if (v.persistentUrl && !urls.includes(v.persistentUrl)) urls.unshift(v.persistentUrl);
                const url = urls.find((u) => isVideoUrl(u, urls)) || urls[0];
                const isVid = !!(url && isVideoUrl(url, urls));
                const failed = isFailedVideoAsset(v, new Set(healthReport?.animaticShots || []));
                return (
                  <div key={v.id} className="cinema-card overflow-hidden">
                        ...
                        <div className="relative">
                          ...
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <div className="text-center px-3">
                              ...
                              <p className="cinema-mono text-[10px] opacity-80 mb-2">视频生成失败 · 显示分镜图</p>
                              <button
                                type="button"
                                disabled={rerenderBusy}
                                onClick={() => void rerenderFailedShots()}
                                className="px-3 py-1.5 rounded-md bg-[var(--cinema-amber)] text-black text-[11px] font-semibold disabled:opacity-50"
                              >
                                补渲失败镜
                              </button>
                            </div>
                    ...
                    <div className="px-3 py-2 flex items-center justify-between gap-2">
                      <span className="cinema-mono text-[10px] tracking-widest text-[var(--cinema-amber)]">
                ...
                {videos.length > 0 ? (
                  <div ref={playerWrapRef} className={`relative ${isPlayerFs ? 'w-screen h-screen bg-black grid place-items-center' : ''}`}>
                    {videos[Math.max(0, playingIndex)]?.mediaUrls?.[0] ? (
                      (() => {
                        const clip = videos[Math.max(0, playingIndex)];
                        const urls: string[] = Array.isArray(clip.mediaUrls) ? clip.mediaUrls.filter(Boolean) : [];
                        if (clip.persistentUrl && !urls.includes(clip.persistentUrl)) urls.unshift(clip.persistentUrl);
                        const url = urls.find((u) => isVideoUrl(u, urls)) || urls[0];
                        const mp = mediaPresentation(isPlayerFs);
                        return isVideoUrl(url, urls) ? (
                          <video
                            key={playingIndex}
                            src={url}
                            autoPlay
      ...
      {regenSbShot && (
        <StoryboardRegenModal
          projectId={id}
          shotNumber={regenSbShot.shotNumber}
          currentImageUrl={regenSbShot.imageUrl}
          currentPrompt={regenSbShot.prompt}
          defaultAspectRatio={'9:16'}
          onComplete={(newUrl) => {
            const sn = regenSbShot.shotNumber;
            setProject((prev: any) => {
              if (!prev) return prev;
              const assets = (prev.assets || []).map((a: any) => {
                if (a.type !== 'storyboard' || a.shotNumber !== sn) return a;
                return {
                  ...a,
                  mediaUrls: [newUrl],
                  persistentUrl: newUrl,
                  data: {
                    ...(a.data || {}),
                    description: regenSbShot.prompt || a.data?.description,
                    prompt: regenSbShot.prompt || a.data?.prompt,
                  },
                };
              });
              return { ...prev, assets: dedupeAssetsByShotNumber(assets.filter((a: any) => a.type === 'storyboard')).concat(assets.filter((a: any) => a.type !== 'storyboard')) };
            });
            setRegenSbShot(null);
          }}
          onCancel={() => setRegenSbShot(null)}
        />
      )}
    </div>
  );
