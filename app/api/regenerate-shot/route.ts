        ...
        if (stage === 'failed-videos' && !shotNumber) {
          ...
          const isDegraded = (a: any) => {
            try {
              ...
              const urls: string[] = JSON.parse(a.media_urls || '[]');
              const candidates = [a.persistent_url, ...urls].map((x) => String(x || '')).filter(Boolean);
              if (d.isAnimatic === true) return true;
              if (candidates.length === 0) return true;
              if (candidates.some((x) => /animatic-\d+\.mp4/.test(x))) return true;
              // 写成了分镜图（无任何 mp4）→ 失败镜，需补渲
              const hasMp4 = candidates.some((x) => /\.(mp4|webm|mov)(\?|#|$)/i.test(x));
              const hasImage = candidates.some((x) => /\.(jpg|jpeg|png|gif|webp)(\?|#|$)/i.test(x));
              return !hasMp4 && hasImage;
            } catch { return false; }
          };
          ...
          await mapPool(targets, 2, async (asset: any) => {
            ...
            try {
              ...
              const result = await orchestrator.regenerateShot(sn, storyboard, {
                duration: 8,
                // 未指定时走 regenerateShot 内百炼优先；勿默认 veo（本环境 Veo OFF）
