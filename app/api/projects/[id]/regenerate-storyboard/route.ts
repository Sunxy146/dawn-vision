...
import { upsertAsset, listAssetsByType, deleteAsset } from '@/lib/repos/asset-repo';
...
async function persistStoryboard(projectId: string, shotNumber: number, imageUrl: string, prompt: string): Promise<void> {
  try {
    // 按镜号 upsert，避免同 SHOT 叠多张卡（旧实现 create 新行「保留历史」）
    await upsertAsset({
      projectId,
      type: 'storyboard',
      name: `镜头 ${shotNumber}`,
    ...
    const same = rows
      .filter((r) => r.shot_number === shotNumber)
      .sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')));
    for (const extra of same.slice(1)) {
      await deleteAsset(extra.id);
    }
  } catch (e) {
...
export async function POST(
  ...
  if (!shotNumber || typeof shotNumber !== 'number') {
    return NextResponse.json({ message: 'shotNumber required' }, { status: 400 });
  }
  if (!customPrompt || typeof customPrompt !== 'string' || customPrompt.trim().length < 5) {
    return NextResponse.json({ message: 'customPrompt too short (min 5 chars)' }, { status: 400 });
  }
  // 分镜规划常 >2000 字；硬拒会变成前端「失败 400」。超长自动截断。
  const MAX_PROMPT = 2500;
  let trimmedPrompt = customPrompt.trim();
  if (trimmedPrompt.length > MAX_PROMPT) {
    trimmedPrompt = trimmedPrompt.slice(0, MAX_PROMPT);
  }
  ...
  if (referenceImageUrl && typeof referenceImageUrl === 'string') {
    if (!referenceImageUrl.startsWith('http')) {
      return NextResponse.json({ message: 'referenceImageUrl must be http URL (upload first)' }, { status: 400 });
    }
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const finalPrompt = optimizeMidjourneyPrompt(trimmedPrompt);
