...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const title = (script.title || `DawnVision ${projectId.slice(0, 8)}`).toString().slice(0, 64);
  const aaf = buildAAF(shots, fps, title);

  return new Response(new Uint8Array(aaf), {
    headers: {
      'Content-Type': 'application/octet-stream',
      'Content-Disposition': `attachment; filename="dawn-vision-${projectId.slice(0, 8)}.aaf"`,
