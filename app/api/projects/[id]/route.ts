...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 同镜号历史重复行（重生曾 insert）→ 合并后只返回一条，避免分镜 Tab 叠卡
  const displayAssets = [
    ...dedupeAssetsByShotNumber(parsedAssets.filter((a: any) => a.type === 'storyboard')),
    ...dedupeAssetsByShotNumber(parsedAssets.filter((a: any) => a.type === 'video')),
    ...parsedAssets.filter((a: any) => a.type !== 'storyboard' && a.type !== 'video'),
  ];
