...
export default function CreatePage() {
  ...
  const handleCameoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { compressImageForUpload } = await import('@/lib/compress-image-client');
      ...
      if (file.size > 10 * 1024 * 1024) {
        showToast({
          title: `已自动压缩 ${(file.size / 1024 / 1024).toFixed(1)}MB → ${(compressed.size / 1024 / 1024).toFixed(1)}MB`,
          type: 'info',
        });
      }
      ...
      reader.onload = () => {
        ...
        setCameoFile(compressed);
        setCameoPreview(dataUri);
        runCameoPreviewScore(dataUri);
      };
      reader.readAsDataURL(compressed);
    } catch (err) {
      showToast({ title: err instanceof Error ? err.message : '图片处理失败', type: 'error' });
    }
  };
