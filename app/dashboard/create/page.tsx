...
import { useProjectWorkspaceStore, useActiveGenerationStore, rememberLastWorkshop, readLastWorkshop, clearLastWorkshop } from '@/lib/store';
...
import {
  ...
  buildResumeChatMessages,
  inferPhaseFromAssets,
} from '@/lib/resume-workshop';
import type { ProjectAsset } from '@/types/agents';

/** 项目页「进入工坊?continue=1」恢复后待执行的续跑任务 */
let pendingContinueVideo: { project: Project; idea: string } | null = null;
...
export default function DashboardCreatePage() {
  ...
  const [lastWorkshopHint, setLastWorkshopHint] = useState<{ projectId: string; idea: string } | null>(null);
  const { showToast } = useToast();

  const stylePreviews = useStylePreviews();
  const {
    setCurrentProject, setNodes, setEdges, setIsProducing, isProducing,
    addChatMessage, setAssets,
  } = useProjectWorkspaceStore();

  // 恢复工坊：?resume=id | 进行中任务 | 最近一次工坊（侧栏点「创作工坊」也能回画布）
  // ?new=1 强制打开创意输入页，不自动恢复
  useEffect(() => {
    if (searchParams.get('new') === '1') return;
    const fromQuery = searchParams.get('resume');
    useActiveGenerationStore.getState().hydrate();
    const active = useActiveGenerationStore.getState().current;
    const last = readLastWorkshop();
    const resumeId = fromQuery || active?.projectId || last?.projectId || null;
    if (!resumeId || workspaceProject?.id === resumeId) return;
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/projects/${encodeURIComponent(resumeId)}`, {
          credentials: 'include',
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
        if (!res.ok) throw new Error(res.status === 404 ? '项目不存在' : '无法加载项目');
        const data = await res.json();
        if (cancelled) return;

        const assets: ProjectAsset[] = (data.assets || []).map((a: any) => ({
          id: a.id,
          projectId: resumeId,
          type: a.type,
          name: a.name,
          ...
          persistentUrl: a.persistentUrl,
          shotNumber: a.shotNumber,
          version: a.version,
          confirmed: !!a.confirmed,
          createdAt: a.createdAt || a.updatedAt || data.createdAt,
          updatedAt: a.updatedAt || data.updatedAt,
        }));

        const project: Project = {
          id: data.id,
          userId: data.userId || data.user_id || 'current-user',
          title: data.title || '未命名项目',
          description: data.description,
          status: data.status || 'active',
          styleId: data.styleId || undefined,
          scriptData: data.scriptData || undefined,
          directorNotes: data.directorNotes || undefined,
          createdAt: data.createdAt,
          updatedAt: data.updatedAt,
        };

        const stillRunning = !!active && active.projectId === resumeId;
        const phase = stillRunning ? active!.phase : inferPhaseFromAssets(assets);
        const incomplete = data.status !== 'completed' && phase !== '完成';
        const hasSb = assets.some((a) => a.type === 'storyboard');
        const hasVid = assets.some((a) => a.type === 'video' && (a.mediaUrls?.[0] || (a as any).persistentUrl));
        const needsVid = hasSb && !hasVid;

        useProjectWorkspaceStore.getState().clearAgentOutputs();
        setCurrentProject(project);
        setAssets(assets);
        const nodes = applyProgressToNodes(
          buildInitialNodes(assets),
          assets,
          incomplete ? phase : null,
        );
        setNodes(nodes);
        setEdges(initialEdges);
        // 刷新/恢复时没有活 SSE：先解锁按钮；若有未完成的「生成视频」任务，下面会自动续跑
        setIsProducing(false);
        setWorkspaceProject(project);
        rememberLastWorkshop(resumeId, data.description || data.title || '');

        if (needsVid && stillRunning) {
          // 上次点过续跑后离开/刷新 → 保留任务，回工坊自动接着跑（避免又变回「继续生成视频」干等）
          useActiveGenerationStore.getState().setPhase('生成视频');
          pendingContinueVideo = {
            project,
            idea: data.description || data.title || active?.idea || '',
          };
        } else if (stillRunning) {
          useActiveGenerationStore.getState().setPhase(phase);
        } else {
          useActiveGenerationStore.getState().finish();
        }

        for (const { role, message } of buildResumeChatMessages(
          resumeId,
          assets,
        ...
        // ?continue=1：标记待续跑，由下方 effect 执行（避免在 resume effect 里提前引用续跑函数）
        // 导演台重跑也会带 continue=1；此时分镜可能已被清掉，不能再要求 needsVid。
        if (!cancelled && searchParams.get('continue') === '1') {
          try {
            window.history.replaceState(null, '', `/dashboard/create?resume=${encodeURIComponent(resumeId)}`);
          } catch { /* ignore */ }
          pendingContinueVideo = {
            project,
            idea: data.description || data.title || '',
          };
        }
      } catch (e) {
        if (!cancelled) {
          // 最近项目已删 → 清记忆，落到创意输入页
          if (!fromQuery) clearLastWorkshop();
          showToast({
            title: e instanceof Error ? e.message : '恢复工坊失败',
            type: 'error',
          });
        }
      } finally {
  ...
  const leaveWorkshopToNew = () => {
    setWorkspaceProject(null);
    setIsProducing(false);
    useProjectWorkspaceStore.getState().resetWorkspace();
    const last = readLastWorkshop();
    setLastWorkshopHint(last ? { projectId: last.projectId, idea: last.idea || '' } : null);
    router.push('/dashboard/create?new=1');
  };
  ...
  /** 续跑已有项目：保留资产，create-stream 按 checkpoint 跳到视频 */
  const continueExistingPipeline = async (overrideProject?: Project, overrideIdea?: string) => {
    // 注意：不能直接绑 onClick={continueExistingPipeline}，否则首参会是 MouseEvent
    const maybe = overrideProject as any;
    const safeProject =
      maybe && typeof maybe === 'object' && typeof maybe.id === 'string' && !maybe.nativeEvent
        ? (overrideProject as Project)
        : undefined;
    const proj = safeProject || workspaceProject || useProjectWorkspaceStore.getState().currentProject;
    if (!proj?.id) {
      showToast({ title: '无法续跑：未找到项目', type: 'error' });
      return;
    }
    if (useProjectWorkspaceStore.getState().isProducing) {
      // 假「生成中」：强制解锁再跑
      setIsProducing(false);
    }
    const idea = sanitizeInput(
      (typeof overrideIdea === 'string' ? overrideIdea : '') || proj.description || proj.title || '继续生成',
    );
    const projectId = proj.id;

    setIsProducing(true);
    useActiveGenerationStore.getState().start({ projectId, idea });
    useActiveGenerationStore.getState().setPhase('生成视频');
    useProjectWorkspaceStore.getState().updateNodeData('node-video', { status: 'running', progress: 10 });
    useProjectWorkspaceStore.getState().addChatMessage(AgentRole.VIDEO_PRODUCER, {
      id: `msg-continue-vid-${Date.now()}`,
      projectId,
      agentRole: AgentRole.VIDEO_PRODUCER,
      role: 'assistant',
      content: '从断点续跑：已有分镜将跳过，开始图生视频…',
      createdAt: new Date().toISOString(),
    });
    ...
    const isAbort = (error: unknown) => {
      ...
      if (typeof error === 'object' && error !== null && 'name' in error) {
        const n = String((error as { name?: string }).name || '');
        if (n === 'AbortError') return true;
      }
      const msg = error instanceof Error ? error.message : String(error);
      return /aborted|AbortError|BodyStreamBuffer/i.test(msg);
    };

    try {
      const token = getToken();
      console.log('[ContinueVideo] POST /api/create-stream', projectId);
      const response = await fetch('/api/create-stream', {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      // 正常跑完：有视频则清任务；仍无视频则保留「生成视频」方便再进工坊自动续
      const hasVid = useProjectWorkspaceStore.getState().assets.some(
        (a) => a.type === 'video' && (a.mediaUrls?.[0] || (a as any).persistentUrl),
      );
      if (hasVid) useActiveGenerationStore.getState().finish();
      else useActiveGenerationStore.getState().setPhase('生成视频');
    } catch (error) {
      if (isAbort(error)) {
        // 去「我的项目」等导致 fetch 中断：保留任务标记，下次进工坊自动续跑
        useActiveGenerationStore.getState().setPhase('生成视频');
      } else {
        showToast({ title: error instanceof Error ? error.message : '续跑失败', type: 'error' });
        useActiveGenerationStore.getState().setPhase('生成视频');
      }
    } finally {
      setIsProducing(false);
  ...
  const continueRef = useRef(continueExistingPipeline);
  continueRef.current = continueExistingPipeline;

  // 消费 pendingContinueVideo（来自 ?continue=1 恢复）
  useEffect(() => {
    if (!workspaceProject?.id || resumeLoading) return;
    const job = pendingContinueVideo;
    if (!job || job.project.id !== workspaceProject.id) return;
    pendingContinueVideo = null;
    void continueRef.current(job.project, job.idea);
  }, [workspaceProject?.id, resumeLoading]);
  ...
  const handleSSEEvent = (event: any, projectId: string) => {
    ...
    switch (type) {
      ...
      case 'storyboardPlans': {
        // 第1阶段：纯文本分镜描述（暂无图片）— 同镜号 upsert，避免叠卡
        ...
        (data || []).forEach((sb: any, i: number) => {
          ...
          const patch = {
            ...
            duration: (ex?.data as any)?.duration ?? 10,
          };
          if (ex) {
            s.updateAsset(ex.id, { data: { ...ex.data, ...patch } });
          } else {
            s.addAsset({ id: `asset-sb-${Date.now()}-${i}`, projectId, type: 'storyboard', name: `镜头 ${sn}`, data: patch, mediaUrls: [], shotNumber: sn, version: 1, createdAt: ts, updatedAt: ts });
          }
        });
  ...
  if (workspaceProject) {
    return (
      <CreationWorkspace
        ...
        onNewCreation={leaveWorkshopToNew}
        onContinueVideo={() => { void continueExistingPipeline(); }}
        continueBusy={isProducing}
      />
    );
  }

  if (resumeLoading || (searchParams.get('resume') && searchParams.get('new') !== '1')) {
    return (
      <div className="min-h-[50vh] grid place-items-center text-sm text-white/50">
        正在恢复工坊工作区…
      </div>
    );
  }
  ...
  return (
    <div className="cinema-page -mx-[5vw] -my-6 px-[5vw] py-6">
      ...
      {searchParams.get('new') === '1' && lastWorkshopHint && (
        <button
          type="button"
          onClick={() => router.push(`/dashboard/create?resume=${encodeURIComponent(lastWorkshopHint.projectId)}`)}
          className="mb-4 w-full text-left rounded-lg border border-[var(--cinema-amber)]/40 bg-[var(--cinema-amber)]/10 px-4 py-3 text-[12px] text-[var(--cinema-text)] hover:bg-[var(--cinema-amber)]/15 transition-colors"
        >
          <span className="font-medium text-[var(--cinema-amber)]">返回上次工坊</span>
          <span className="opacity-60"> · {lastWorkshopHint.idea?.slice(0, 40) || lastWorkshopHint.projectId}</span>
        </button>
      )}
      <DemoModeBanner />
