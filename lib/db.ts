  ...
  try {
    ...
    const run = db.transaction(() => {
      ...
      db.prepare(`INSERT INTO users (id, email, password_hash, name, role, avatar_url, locale, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
        demoUserId, 'demo@qfmanju.ai', passwordHash, '晓阳叙影 Demo', demoRole, AVATAR, 'zh', now()
      );
      ...
      const demoProjects = [
        ...
        { title: '风起晓阳叙影', description: '多镜头分镜与氛围光影测试。', covers: [seedSvg(300, 180, '#1e3a5f', '#4de0c2', '风起晓阳叙影'), seedSvg(300, 180, '#0f172a', '#ef319f', '风起2')], status: 'completed' },
      ];
      ...
      const demoCases: { title: string; category: string; cover: string; author: string; video?: string }[] = [
        { title: '月华藏境', category: '东方幻想', cover: seedSvg(400, 300, '#312e81', '#f9a8d4', '月华藏境'), author: '晓阳叙影 Studio', video: '/cases/clip-c.mp4' },
        { title: '霓虹回响', category: '赛博都市', cover: seedSvg(400, 300, '#0c4a6e', '#ef319f', '霓虹回响'), author: 'DawnVision Lab', video: '/cases/clip-a.mp4' },
        { title: '星潮旅人', category: '科幻冒险', cover: seedSvg(400, 300, '#1e1b4b', '#4de0c2', '星潮旅人'), author: '晓阳叙影 Studio', video: '/cases/clip-b.mp4' },
        { title: '云岚日记', category: '治愈日常', cover: seedSvg(400, 300, '#064e3b', '#a78bfa', '云岚日记'), author: 'DawnVision Lab', video: '/cases/clip-d.mp4' },
      ];
