...
/** 最近一次打开的工坊项目 —— finish() 后仍保留，侧栏点「创作工坊」可直接回到画布 */
const LAST_WORKSHOP_KEY = 'qfmj-last-workshop';

export function rememberLastWorkshop(projectId: string, idea?: string) {
  if (typeof window === 'undefined' || !projectId) return;
  try {
    localStorage.setItem(LAST_WORKSHOP_KEY, JSON.stringify({
      projectId,
      idea: idea || '',
      at: Date.now(),
    }));
...
export function readLastWorkshop(): { projectId: string; idea: string; at: number } | null {
  if (typeof window === 'undefined') return null;
  try {
    const raw = localStorage.getItem(LAST_WORKSHOP_KEY);
    if (!raw) return null;
...
export function clearLastWorkshop() {
  if (typeof window === 'undefined') return;
  try { localStorage.removeItem(LAST_WORKSHOP_KEY); } catch { /* ignore */ }
}
...
export const useActiveGenerationStore = create<ActiveGenerationStore>((set, get) => ({
  ...
  start: ({ projectId, idea }) => {
    ...
    persistActiveGen(v);
    rememberLastWorkshop(projectId, idea);
    set({ current: v });
  },
  setPhase: (phase) => {
