...
import { loadCheckpoints, checkpointSummary, type PipelineCheckpoints } from '@/lib/pipeline-checkpoints';
...
export interface CreatePipelineInput {
  ...
  sketchLock?: boolean;
  /** 已鉴权用户 id；写入 projects.user_id，避免归属落到库里第一个用户 */
  ownerUserId?: string;
}
...
export async function runCreatePipeline(input: CreatePipelineInput, emit: PipelineEmit, opts?: { resume?: boolean }): Promise<void> {
  const { idea, projectId, videoProvider, style, aspect, enableGates, templateId, primaryCharacterRef, lockedCharacters, cameraDefault, previewSeedImage, references, replicaScript, editStyle, language, sketchLock, ownerUserId } = input as CreatePipelineInput & Record<string, any>;
  ...
  try {
    ...
    // 获取归属用户 —— 优先用入口鉴权的 ownerUserId；否则冷启动兜底
    let userId = typeof ownerUserId === 'string' && ownerUserId ? ownerUserId : '';
    try {
      if (!userId) {
        ...
        if (user) {
          ...
          const demo = await createUser({ email: 'demo@dawnvision.local', passwordHash: 'dummy', name: '演示用户', role: 'user' });
          userId = demo.id;
    ...
    // ── v10.4.2 幂等续跑:同 projectId「有则跳过」(不重复生成/计费)──
    // 此前仅 opts.resume / job.attempts>1 才装载 → 工坊「继续生成视频」带已有 projectId
    // 却仍 emptyCheckpoints，整条管线从导演/编剧重跑。现改为始终装载断点。
    const cp: PipelineCheckpoints = await loadCheckpoints(projectId);
    {
      const summary = checkpointSummary(cp);
      if (summary !== '无') {
        console.log(`[Resume] ${projectId} 断点装载: ${summary}`);
        send('status', { message: `[续跑] 已装载断点产物:${summary}` });
      } else if (opts?.resume) {
        console.log(`[Resume] ${projectId} 断点装载: 无`);
        send('status', { message: '[续跑] 未发现可复用断点,将从头生成' });
      }
    }
    ...
    } else if (cp.storyboards.length > 0) {
      // 续跑已有分镜时不再卡 Style Bible（常要等超时才放行）
      const fallback = cp.storyboards.find((s: any) => s.imageUrl)?.imageUrl || '';
      send('step', { step: 'styleBible' });
      if (fallback) send('styleBible', { url: fallback });
      send('status', { message: `[续跑] 已有分镜图×${cp.storyboards.length},跳过 Style Bible 渲染` });
      console.log(`[Resume] ${projectId} 跳过 Style Bible（已有分镜×${cp.storyboards.length}）`);
    } else try {
      send('step', { step: 'styleBible' });
