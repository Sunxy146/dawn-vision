...
/** 可作为分镜首帧/成片引用的 URL(含本地 serve-file、data:)。 */
function isUsableMediaUrl(u: string): boolean {
  return (
    /^https?:\/\//i.test(u)
    || /^data:image\//i.test(u)
...
/** persistent_url 优先;否则 media_urls 第一条可用地址。 */
export function assetUrl(row: AssetRow): string {
  if (row.persistent_url && isUsableMediaUrl(row.persistent_url)) return row.persistent_url;
  if (row.persistent_url) return row.persistent_url; // 相对 key / 其它本地路径仍可用
  try {
    const urls: string[] = row.media_urls ? JSON.parse(row.media_urls) : [];
    return urls.find((u) => typeof u === 'string' && isUsableMediaUrl(u)) || '';
  } catch { return ''; }
}
...
export async function loadCheckpoints(projectId: string): Promise<PipelineCheckpoints> {
  ...
  // 资产表无 script 时,回退 projects.script_data(工坊页展示剧本的同源字段)
  if (!cp.script) {
    try {
      const prow = db.prepare('SELECT script_data FROM projects WHERE id = ?').get(projectId) as
        | { script_data?: string | null }
        | undefined;
      if (prow?.script_data) {
        const s = JSON.parse(prow.script_data);
        if (Array.isArray(s?.shots) && s.shots.length > 0) cp.script = s;
      }
    } catch { /* ignore */ }
  }
  // 有剧本无导演计划资产 → 用剧本合成最小 plan,避免续跑再烧一次导演 LLM
  if (!cp.plan && cp.script) {
    cp.plan = {
      title: cp.script.title || '续跑项目',
      logline: cp.script.synopsis || '',
      synopsis: cp.script.synopsis || '',
      genre: cp.script.theme || '',
      visualStyle: '',
      characters: [],
      scenes: [],
      _resumedFromScript: true,
    };
  }
  ...
  // 成片 URL 以 final_video 资产为准（补渲/脚本合成后可能比 timeline.data 更新）
  if (finalRows.length) {
    const finalUrl = assetUrl(finalRows[finalRows.length - 1]);
    if (finalUrl) {
      if (cp.editResult && typeof cp.editResult === 'object') {
        cp.editResult = { ...cp.editResult, finalVideoUrl: finalUrl };
      } else {
        cp.editResult = {
          timeline: [],
          totalDuration: 0,
