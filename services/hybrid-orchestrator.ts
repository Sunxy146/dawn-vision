...
export class HybridOrchestrator {
  ...
  private async callLLM(systemPrompt: string, userMessage: string, json = true, useCreativeModel = false, opts?: { maxTokens?: number; timeoutMs?: number }): Promise<string> {
    ...
    // 调用方常硬编码 16384/24576;百炼兼容模式上限 8192 —— 设了 OPENAI_MAX_TOKENS 就当硬顶,
    // 否则超限直接 400,队列任务空转、项目页只剩「制作中」看不到进度。
    let maxTokens = opts?.maxTokens ?? defaultMaxTokens;
    if (Number.isFinite(envDefault) && envDefault > 0) maxTokens = Math.min(maxTokens, envDefault);
  ...
  private async generateImage(prompt: string, opts?: {
    ...
    // 百炼直连优先:Turbopack 下 plugin registry 可能双实例,注册了也不进 withImagePlugin。
    // DASHSCOPE_IMAGE_ENABLED=1 时先打原生 multimodal-generation,失败再走旧链。
    try {
      const { hasDashscopeImage, generateDashscopeImage } = await import('@/lib/image-providers/dashscope-qwen-image');
      if (hasDashscopeImage()) {
        ...
        if (r?.imageUrl) {
          console.log(`[ImageRouter] ✅ dashscope-qwen-image for: ${opts?.label || 'image'}`);
          return r.imageUrl;
        }
      }
    } catch (e) {
      console.warn(`[ImageRouter] dashscope-qwen-image failed for ${opts?.label || 'image'}:`, e instanceof Error ? e.message : e);
    }
  ...
  private async doLegacyGenerateImage(prompt: string, opts?: {
    ...
    // 站内 /api/serve-file 锁脸图 → data:/http，否则外部引擎拉不到、旧逻辑也会丢 ref
    if (opts?.cref || opts?.sref || opts?.referenceImages?.length) {
      try {
        const { collectUsableImageRefs, materializeImageRefs } = await import('@/lib/image-refs');
        const raw = collectUsableImageRefs({
          cref: opts.cref,
          sref: opts.sref,
          referenceImages: opts.referenceImages,
        }, 4);
        const mats = await materializeImageRefs(raw);
        if (mats.length > 0) {
          opts = {
            ...opts,
            cref: mats[0],
            referenceImages: mats,
            // 画风位尽量保留末张（若原 sref 在链上）；否则交给 referenceImages
            sref: mats.length > 1 ? mats[mats.length - 1] : mats[0],
          };
        }
      } catch (e) {
        console.warn('[ImageRouter] materialize refs failed:', e instanceof Error ? e.message : e);
      }
    }
...
${shots.map((s, i) => {
  ...
  async runVideoProducer(
    ...
    const generateSingleVideo = async (board: Storyboard, i: number): Promise<VideoClip> => {
      ...
      // 远程视频 API：多数要公网 http；百炼 HappyHorse 也接受 data:image base64
      const hasCharRef = characterRefUrl && characterRefUrl.startsWith('http');
      const hasFirstFrame = !!(firstFrameUrl && (
        firstFrameUrl.startsWith('http') || firstFrameUrl.startsWith('data:image')
      ));
      ...
      // 百炼 I2V 直连优先（与 ImageRouter 同理：绕过 PLUGIN_CHAIN_MODE=off / Turbopack 双实例）
      let usedVideoEngine = ''; // v12.4.0: 记下真正出片的引擎
      let dashVideoUrl = '';
      if (hasFirstFrame && firstFrameUrl) {
        try {
          const { hasDashscopeVideo, generateDashscopeI2v } = await import('@/lib/video-providers/dashscope-i2v');
          if (hasDashscopeVideo()) {
            const designed = typeof (shot as any)?.duration === 'number' ? (shot as any).duration : 5;
            const r = await generateDashscopeI2v({
              prompt: enhancedPrompt,
              firstFrameUrl,
              durationSec: Math.max(3, Math.min(15, Math.round(designed || 5))),
              aspectRatio: this.videoAspect() as '16:9' | '9:16' | '1:1',
              label: `shot-${board.shotNumber}`,
              onProgress: (pct, msg) => {
                this.update(AgentRole.VIDEO_PRODUCER, {
                  status: 'working',
                  currentTask: msg || `百炼图生视频 ${Math.round(pct * 100)}%`,
                  progress: Math.min(95, Math.round(pct * 100)),
                });
                this.emit('videoProgress', {
                  shotNumber: board.shotNumber,
                  progress: Math.round(pct * 100),
                  status: msg || 'dashscope-i2v',
                });
              },
            });
            if (r?.videoUrl) {
              console.log(`[Video] ✅ dashscope-i2v for shot ${board.shotNumber}: ${r.videoUrl.slice(0, 80)}`);
              dashVideoUrl = r.videoUrl;
              usedVideoEngine = 'dashscope-i2v';
            }
          }
        } catch (e) {
          console.warn(`[Video] dashscope-i2v failed for shot ${board.shotNumber}:`, e instanceof Error ? e.message : e);
        }
      }
      ...
      let videoUrl = dashVideoUrl;
      if (videoUrl) {
        ranVideoProvider = 'dashscope-i2v';
      } else {
        videoUrl = await withVideoPlugin(
          {
            prompt: enhancedPrompt,
...
`;
  ...
  async regenerateShot(shotNumber: number, storyboard: Storyboard, options?: { duration?: number; videoProvider?: string; tailFrameUrl?: string }): Promise<VideoClip> {
    ...
    // ── 百炼 I2V/R2V 优先（主管线同款模型池；此前补渲只走 Veo/Minimax/Kling，全 OFF 时 335ms 假失败）──
    if (engineFrame && (engineFrame.startsWith('http') || engineFrame.startsWith('data:image') || engineFrame.startsWith('/api/serve-file'))) {
      try {
        const { hasDashscopeVideo, generateDashscopeI2v } = await import('@/lib/video-providers/dashscope-i2v');
        if (hasDashscopeVideo()) {
          const r = await generateDashscopeI2v({
            prompt: storyboard.prompt || `镜头 ${shotNumber}`,
            firstFrameUrl: engineFrame,
            durationSec: Math.max(3, Math.min(10, Math.round(duration))),
            aspectRatio: this.videoAspect() as '16:9' | '9:16' | '1:1',
            label: `regen-shot-${shotNumber}`,
            onProgress: (pct, msg) => {
              this.update(AgentRole.VIDEO_PRODUCER, {
                status: 'working',
                currentTask: msg || `补渲镜 ${shotNumber} ${Math.round(pct * 100)}%`,
                progress: Math.min(95, Math.round(pct * 100)),
              });
              this.emit('videoProgress', {
                shotNumber,
          ...
          if (r?.videoUrl) {
            console.log(`[Regenerate] Shot ${shotNumber} ✅ dashscope-i2v: ${r.videoUrl.slice(0, 80)}`);
            this.update(AgentRole.VIDEO_PRODUCER, { status: 'completed', progress: 100 });
            return { shotNumber, videoUrl: r.videoUrl, duration: r.durationSec || duration, status: 'completed', isAnimatic: false } as VideoClip & { isAnimatic: boolean };
          }
        }
      } catch (e) {
        console.warn(`[Regenerate] dashscope-i2v failed for shot ${shotNumber}:`, e instanceof Error ? e.message.slice(0, 160) : e);
      }
    }
    ...
    if (!videoUrl) {
      ...
      try {
        ...
        console.error(`[Regenerate] Shot ${shotNumber} Ken Burns 也失败:`, msg.slice(0, 160));
        throw new Error(`镜头 ${shotNumber} 补渲失败：百炼/备用引擎均未出片，且本地动效不可用(${msg.slice(0, 80)})`);
      }
    }
    this.update(AgentRole.VIDEO_PRODUCER, { status: 'completed', progress: 100 });
