...
transitionDuration: 0.0-1.5 (cut 类用 0, fade 类用 0.5-1.2)`,
    ...
    // 成片必须落成 ?key=：?path=&sig= 依赖登录 cookie + HMAC，<video src> 常 401/403 →「本地合成视频文件已失效」
    if (finalVideoUrl.startsWith('/api/serve-file?path=')) {
      try {
        const { persistAsset } = await import('@/lib/asset-storage');
        const persisted = await persistAsset(finalVideoUrl, { contentType: 'video/mp4', ext: '.mp4' });
        if (persisted?.url) {
          console.log(`[Editor] Final video persisted: ${persisted.url}`);
          finalVideoUrl = persisted.url;
        } else {
          console.warn('[Editor] Final video persist skipped (null); playback may require auth for ?path=');
        }
      } catch (e) {
        console.warn('[Editor] Final video persist failed:', e instanceof Error ? e.message : e);
      }
    }
