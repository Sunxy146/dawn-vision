...
function downloadFile(url: string, destPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    // 处理 /api/serve-file(?path= / ?key=) —— 直接拷本地文件，避免走 HTTP
    if (url.startsWith('/api/serve-file')) {
      try {
        const { serveFileToLocalPath } = require('@/lib/first-frame') as typeof import('@/lib/first-frame');
        const localPath = serveFileToLocalPath(url);
        if (localPath && fs.existsSync(localPath)) {
          fs.copyFileSync(localPath, destPath);
          console.log(`[Download] /api/serve-file → local copy: ${localPath}`);
          return resolve();
        }
        // 兼容旧逻辑：仅 path= 且未验签时的直解（本地脚本/无签名）
        const u = new URL(url, 'http://localhost');
        const rawPath = decodeURIComponent(u.searchParams.get('path') || '');
        if (rawPath && fs.existsSync(rawPath)) {
          fs.copyFileSync(rawPath, destPath);
          console.log(`[Download] /api/serve-file?path= → local copy: ${rawPath}`);
          return resolve();
        }
      } catch (e) {
...
export async function composeVideo(options: ComposeOptions): Promise<ComposeResult> {
  ...
  // 音轨策略：
  // - nativeAudioShots 明示的镜 → 用成片真音轨
  // - 其余镜：若该镜没有单独配音轨，且源片 ffprobe 有音轨 → 同样保留（百炼/I2V 常自带环境声）
  // - 有 TTS 配音的镜仍铺静音再叠配音，避免双口轨
  // 此前默认全 anullsrc → 无 BGM/TTS 时成片「有音轨但 4kbps 静音」
  const voShotSet = new Set((voiceoverClips || []).map((v) => v.shotNumber));
  const clipHasNativeAudio: boolean[] = new Array(localClips.length).fill(false);
  for (let i = 0; i < localClips.length; i++) {
    const sn = validClips[i]?.shotNumber as number;
    const preferNative = nativeShotSet.has(sn) || (typeof sn === 'number' && !voShotSet.has(sn));
    if (!preferNative) continue;
    try {
      ...
      clipHasNativeAudio[i] = false;
    }
    if (nativeShotSet.has(sn) && !clipHasNativeAudio[i]) {
      console.warn(`[Composer] 镜 ${sn} 标记原生音频但成片无音轨 → 兜底静音`);
    }
  }
  const kept = clipHasNativeAudio.filter(Boolean).length;
  if (kept > 0) console.log(`[Composer] 保留 ${kept}/${localClips.length} 镜源片音轨`);
