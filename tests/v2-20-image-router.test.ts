 ...
 *   - fallback 链按可用引擎全部排好
 *   - collectValidRefs 去重 + 保留 http/data:/serve-file
 */
...
describe('v2.20 P0.3 · collectValidRefs', () => {
  ...
  it('keeps data: and serve-file refs (Cameo local locks)', () => {
    const out = collectValidRefs({
      cref: 'data:image/png;base64,xxx',
      sref: 'https://b.png',
      referenceImages: ['/api/serve-file?key=abc', 'https://c.png'],
    });
    expect(out).toEqual([
      'data:image/png;base64,xxx',
      'https://b.png',
      '/api/serve-file?key=abc',
      'https://c.png',
    ]);
  });
