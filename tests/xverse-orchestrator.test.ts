...
function buildMockScript(shotCount: number) {
  return {
    title: '晓阳叙影别师',
    logline: '当晓阳叙影遭遇师门反叛，她必须做出抉择，否则信念将永远破碎',
    ...
    characterArcs: [{
      name: '晓阳叙影', arc: '从顺从到觉醒', desire: '回归师门',
  ...
  it('XVERSE_ENABLED=true 时 runWriter 直接走 XVerse 主路径', async () => {
    ...
    xverseSvc.__setTransport(async (payload) => {
      ...
      if (sys.includes('精通分镜')) {
        ...
        for (let i = 1; i <= 5; i++) lines.push(`镜头${i}: 场景${i} - 节拍 - 角色:晓阳叙影 - 台词:"x"`);
    ...
    expect(script.title).toBe('晓阳叙影别师');
